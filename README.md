...
- **One query observer per page for a given key.** Mounting a second `useCurrentUser()` observer inside a page that's already behind `GuestRoute`/`ProtectedRoute` caused a genuine infinite-refetch bug during development (two observers of an erroring, non-401, `retry: false` query that never navigates away). It also caused a quieter, non-looping version of the same mistake on `/dashboard`: a second observer there fired one redundant `GET /auth/me` on every load, since the `QueryClient` has no `staleTime`. `RegisterPage` and `DashboardPage` both read the guard's already-mounted query via `getQueryData` instead of calling the hook a second time — see the note in `CLAUDE.md` for the full story.
